...
class Dense3(Layer):
    ...
    def __init__(self, input_dim, output_dim, dropout=0., act=tf.nn.relu, share_user_item_weights=False,
                 bias=False, **kwargs):
        super(Dense3, self).__init__(**kwargs)
...
class StackGCN3D(Layer):
    """Graph convolution layer for bipartite graphs and sparse inputs."""

    def __init__(self, input_dim, output_dim, support, support_t, num_support,e_f_u,e_f_v,num_context, u_features_nonzero=None,
                 v_features_nonzero=None, sparse_inputs=False, dropout=0.,
                 act=tf.nn.relu, share_user_item_weights=True, **kwargs):
        super(StackGCN3D, self).__init__(**kwargs)
...
class OrdinalMixtureGCN(Layer):
    ...
    def _call(self, inputs):
        ...
        if self.self_connections:   #false
        ...
        for i in range(len(self.support)):
            wu += self.weights_u[i]
            wv += self.weights_v[i]
            print(f"wu {self.weights_u[i].shape}  wv {self.weights_v[i].shape}")
            ...
            print(f"x_u {x_u.shape}  x_u {x_v.shape}")
            print(f"tmp_u {tmp_u.shape}  tmp_v {tmp_v.shape}")

            support = self.support[i]
            support_transpose = self.support_transpose[i]

            a=tf.sparse_tensor_dense_matmul(support, tmp_v)
            b=tf.sparse_tensor_dense_matmul(support_transpose, tmp_u)
        ...
        print(f"z_u {z_u.shape}  z_v {z_v.shape}")
        ...
        print(f"Ya Allah help : Ordinary Mix : u_outputs {u_outputs.shape}-- u_outputs {v_outputs.shape}")
...
class OrdinalMixtureGCN3D(Layer):
    ...
    def __init__(self, input_dim, output_dim, support, support_t, num_support,e_f_u,e_f_v,num_context, u_features_nonzero=None,
                 v_features_nonzero=None, sparse_inputs=False, dropout=0.,
                 act=tf.nn.relu, bias=False, share_user_item_weights=False, self_connections=False, **kwargs):
        super(OrdinalMixtureGCN3D, self).__init__(**kwargs)
    ...
    def _call(self, inputs):
        ...
        for i in range(len(self.support)):
            ...
            print(f"support {support.shape} self.e_f_v {self.e_f_v.shape}")
            print(f"support {support_transpose.shape} self.e_f_v {self.e_f_u.shape}")
            mul=tf.sparse.matmul(support,self.e_f_v)
            mul2=tf.sparse.matmul(support_transpose,self.e_f_u)


            # then multiply with rating matrices

            supports_u.append(dot(mul, tmp_u))  #support

            supports_v.append(dot(mul2, tmp_v))  #support_transpose
...
class GCNC(Layer):
    """Graph convolution layer context between user and item"""

    def __init__(self, input_dim, output_dim, adj_context, adj_context_t, u_features_nonzero=None,
                 v_features_nonzero=None, sparse_inputs=False, dropout=0.,
                 act=tf.nn.relu, bias=False, share_user_item_weights=False, self_connections=False, **kwargs):
        super(GCNC, self).__init__(**kwargs)
        ...
        self.u_features_nonzero = u_features_nonzero
        self.v_features_nonzero = v_features_nonzero
        self.adj_context=adj_context
        self.adj_context_t=adj_context_t
    ...
    def _call(self, inputs):
        ...
        support = self.adj_context
        support_transpose = self.adj_context_t

        # then multiply with rating matrices
        print(f"support {type(support)} tmp_v {type(tmp_v)}")
        supports_u=tf.tensordot(support, tmp_v, axes=1)
        supports_v=tf.tensordot(support_transpose, tmp_u, axes=1)
        ...
        print(f"Ya Allah help : u_outputs {u_outputs.shape}-- u_outputs {v_outputs.shape}")
