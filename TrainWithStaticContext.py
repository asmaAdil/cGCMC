...
from preprocessing import create_trainvaltest_split, \
    sparse_to_tuple, create_trainvaltest_split_Context, preprocess_user_item_features, normalize_edge_features_3D, normalize_edge_features_2D, globally_normalize_bipartite_adjacency, \
    load_data_monti, load_official_trainvaltest_split, normalize_features, globally_normalize_tripartite_adjacency_matrix,user_context_adjacency,item_context_adjacency
#from model import RecommenderGAE, RecommenderSideInfoGAE,  RecommenderEdgeSideInfoGAE
...
ap.add_argument("-d", "--dataset", type=str, default="LDOS",
...
print(f"DATASET {DATASET} {datasplit_path}   ")
if DATASET == 'flixster' or DATASET == 'douban' or DATASET == 'yahoo_music':
    ...
    test_u_indices, test_v_indices, class_values = create_trainvaltest_split_Context(LDcall,DATASET, DATASEED, TESTING,
                                                                             ...
                                                                             VERBOSE)
    print(f"adj_edge_train {adj_edge_train.shape} e_features_train {e_features_train.shape} u_features {u_features}")
else:
...
if not FEATURES:
    ...
    u_features, v_features = preprocess_user_item_features(u_features, v_features)  #just stack (943, 2625) (1682, 2625)
    ...
    print(f"Ya Allah help me please {num_side_features} ")
    ...
    u_features_side = normalize_features(u_features)
    v_features_side = normalize_features(v_features)
    print(f"Ya Allah help me please {u_features_side.shape[1]} - { v_features_side.shape[1]} ")
    u_features_side, v_features_side = preprocess_user_item_features(u_features_side, v_features_side)  #943 x 23 and 1682 x 18 = (943, 41) and (1682, 41)
    u_features_side = np.array(u_features_side.todense(), dtype=np.float32) #(943, 41) (121, 2842)
    v_features_side = np.array(v_features_side.todense(), dtype=np.float32) # (1682, 41)  (1232, 2842)
...
#Depaul has no user and item features
if EDGEFEATURES == True :
        e_features_train_side = normalize_edge_features_3D(adj_edge_train)
        e_features_test_side = normalize_edge_features_3D(adj_edge_test)
        e_features_val_side = normalize_edge_features_3D(adj_edge_val)
        num_edge_side_features = e_features_val_side.shape[2]  # 49
        print(f"Ya Allah help me please {num_side_features}  {e_features_train_side.shape}")
...
print("************************************************working user item context******************")
user_context=user_context_adjacency(support_e)
user_context_sparse = sp.csr_matrix(user_context)

item_context=item_context_adjacency(support_e_t)
item_context_sparse = sp.csr_matrix(item_context)

user_context = normalize_features(user_context_sparse )
item_context = normalize_features(item_context_sparse)

user_context, item_context = preprocess_user_item_features(user_context, item_context)  #943 x 23 and 1682 x 18 = (943, 41) and (1682, 41)

user_context = np.array(user_context.todense(), dtype=np.float32) #(943, 41) (121, 2842)
item_context = np.array(item_context.todense(), dtype=np.float32) # (1682, 41)  (1232, 2842)

num_context_features = user_context.shape[1]  #41 #2842
print(f"user_context {user_context.shape} type{type (user_context)} item_context {item_context.shape}")
print("*********************************************************************")
...
test_support_e_t=np.reshape(test_support_e,(test_support_e.shape[1],test_support_e.shape[0],test_support_e.shape[2]))
...
if FEATURES:
    ...
    print(f" YA ALLAH MADAD: train_u_features_side {type(train_u_features_side)}  ")
...
if EDGEFEATURES:
    ...
    user_context_test=user_context[np.array(test_u)]
    item_context_test = item_context[np.array(test_v)]

    val_e_features_side = e_features_val_side
    user_context_val=user_context[np.array(val_u)]
    item_context_val = item_context[np.array(val_v)]

    train_e_features_side = e_features_train_side
    user_context_train=user_context[np.array(train_u)]
    item_context_train = item_context[np.array(train_v)]
    print(f" YA ALLAH MADAD:    user_context_train {type(user_context_train)} ")
...
if DATASET=='LDOS' or DATASET=='Travel_STS':
 print("I am called for placeholder Travel STS")
 placeholders = {
 ...
 print("I am called for placeholder for DePaul")
 placeholders = {
    'u_features': tf.sparse_placeholder(tf.float32, shape=np.array(u_features.shape, dtype=np.int64)),
    'v_features': tf.sparse_placeholder(tf.float32, shape=np.array(v_features.shape, dtype=np.int64)),
...
EDGEHIDDEN=10
testing=True
print("Ya Allah help me in achieving")
...
if FEATURES:
    ...
    if (DATASET=='LDOS' or DATASET=='Travel_STS') and ContextFeatures==False:
    ...
    elif (DATASET=='LDOS' or DATASET=='Travel_STS') and ContextFeatures==True and useboth==False:
      print("I am called RecommenderSideInfoGAE with edge features")
      model = RecommenderContextSideInfoGAE( placeholders,
        ...
        print("I am called RecommenderBothSideInfoGAE with features of user and item along with context")
...
print(f"user_context_train {user_context_train.shape} item_context_train {item_context_train.shape}")
# Feed_dicts for validation and test set stay constant over different update steps
train_feed_dict = construct_feed_dict_e(placeholders, u_features, v_features, user_context_train, item_context_train, u_features_nonzero,
...
val_feed_dict = construct_feed_dict_e(placeholders, u_features, v_features, user_context_val, item_context_val, u_features_nonzero,
...
print("*************************TESTING*********************")
print(f"user_context_test {user_context_test.shape} item_context_test {item_context_test.shape}")
#print(f"test_u_features_side {test_u_features_side.shape} test_v_features_side {test_v_features_side.shape}")
test_feed_dict = construct_feed_dict_e(placeholders, u_features, v_features, user_context_test, item_context_test, u_features_nonzero,
...
for epoch in range(NB_EPOCH):
    ...
    #print(f"outs[0] {outs[0]} ------ outs[1] {outs[1]} ------ outs[2] {outs[2]}")

    val_avg_loss, val_rmse = sess.run([model.loss, model.rmse], feed_dict=val_feed_dict)

    #print(f"val_avg_loss {val_avg_loss} ------ val_rmse {val_rmse} ")
